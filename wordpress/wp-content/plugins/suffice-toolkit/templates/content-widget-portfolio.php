...
while ( $project_query->have_posts() ) : $project_query->the_post();
	...
	if ( has_post_thumbnail() ) {
		...
		$output .= '<a href="' . $image_per . '" ><div class="portfolio-item-img" style="background-image:url(' . get_the_post_thumbnail_url( $post->ID ) . ')"></div></a>';
		$output .= '<figcaption class="portfolio-item-description">';
		$output .= '<p class="portfolio-item-price">' . get_post_meta( $post->ID, '_product_price', true ) . '</p>';
		$output .= '<h5 class="portfolio-item-title"><a href = "' . $image_per . '">' . get_the_title( $post->ID ) . '</a></h5>';
		$output .= '<div class="portfolio-item-more"><a href = "' . $image_per . '">READ MORE<i class="fa fa-long-arrow-right" aria-hidden="true"></i></a></div>';
		$output .= '</figcaption>';
		$output .= '</figure>';
		$output .= '</li>';
	}
